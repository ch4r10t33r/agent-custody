...
- Consumers install compiled output. Each package has `tsconfig.build.json` emitting `dist/` with declarations and an `exports` map in its package.json; a new public module is added to `src/index.ts`, and a new adapter gets its own `./sdk/<name>` subpath. Pack with `bun pm pack`, not `npm pack`, so `workspace:*` dependencies are rewritten to versions. The packages stay `private: true` until the npm scope is claimed.
