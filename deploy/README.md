...
| `ROLE=portal` with `PORTAL_SECRET`, `PORTAL_HOST`, and optionally `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_PRICE_TEAM` | the tenant portal: registration, first key, usage against plan, keys, billing, export; the compose file runs it as the `portal` service and Caddy serves it at `PORTAL_HOST` | unset |
