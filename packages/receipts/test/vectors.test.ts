...
describe("conformance vectors", () => {
  ...
  for (const c of receipts.cases as any[]) {
    it(`receipts: ${c.name}`, () => {
      ...
      const r = verifyBundle(c.bundle, { issuerKeys: c.issuerKeys.map(keyOf), principalKeys: c.principalKeys.map(keyOf), logKeys: c.logKeys.map(keyOf), ...(c.upstreamKeys ? { upstreamKeys: c.upstreamKeys.map(keyOf) } : {}), ...(c.providerSecrets ? { providerSecrets: c.providerSecrets } : {}), ...(c.logId ? { logId: c.logId } : {}), ...(logFile ? { logFile } : {}) });
