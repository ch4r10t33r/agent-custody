...
| `tool.name` | string | `observed` on gateway receipts, `claimed` on SDK receipts | |
...
7. With an upstream signature and an upstream key: `upstream signature (upstream key)`, the envelope verifies against a trusted upstream key and binds to this receipt id, this tool, and this result's content digest.
