...
| `execution` | `{ status: "executed" \| "failed", result, resultDigest }` or `{ status: "denied", reason }` or `{ status: "error", error }`, each with `provenance` | as stated | on a gateway receipt a denial means the call never reached the tool |
...
7. With a policy: `policy decision consistent with execution` (allow never pairs with denied; deny always does) and `no policy errors on an allow`.
8. `tree head signature` against log keys then issuer keys, reporting which signed; `tree head matches inclusion proof size`; `log inclusion proof` recomputes the root from the leaf hash of the canonical envelope and the proof.
9. Optionally, with a copy of the log, `log file root matches tree head` recomputes the root at the tree head's size.
