...
[`@agent-custody/receipts`](/receipts/) on npm implements every object and check on this page, and its test suite is the conformance suite until test vectors are published separately. Verification is the pure function `verifyBundle`; auditing is `auditExtends`.
