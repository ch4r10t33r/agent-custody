...
A log publishes `GET /.well-known/agent-custody-log.json`: `{ log?, keys: [{ keyid, alg: "ed25519", publicKeyPem, validFrom, validTo? }] }`, current key first, retired keys after it, so a verifier fetches keys from the log's own origin and pins them by keyid. A log also publishes checkpoints, signed tree heads at increasing sizes, as files `<tenant>/<treeSize>.json` on a host of their own and at `GET /checkpoints?since=<size>`; each is `{ treeSize, rootHash, signedAt, treeHead }` with `treeHead` the same signed envelope as in a receipt. Any two checkpoints, or a receipt's head and a later checkpoint, are compared with a consistency proof.
