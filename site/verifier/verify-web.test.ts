...
describe("browser verifier against the conformance vectors", () => {
  ...
  for (const c of receipts.cases as any[]) {
    it(`receipts: ${c.name}`, async () => {
      const r = await verifyBundle(c.bundle, { issuerKeys: await all(c.issuerKeys), principalKeys: await all(c.principalKeys), logKeys: await all(c.logKeys), ...(c.log ? { logLeaves: c.log } : {}) });
